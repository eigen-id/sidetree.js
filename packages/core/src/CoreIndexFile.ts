...
export default class CoreIndexFile {
  ...
  public static async parse(
    ...
    try {
      coreIndexFileModel = await JsonAsync.parse(
    ...
    if ('writerLockId' in coreIndexFileModel) {
      if (typeof coreIndexFileModel.writerLockId !== 'string') {
        throw new SidetreeError(
          ErrorCode.CoreIndexFileWriterLockIdPropertyNotString
        );
      }
    ...
    if (operations.deactivate !== undefined) {
      if (!Array.isArray(operations.deactivate)) {
        throw new SidetreeError(
          ErrorCode.CoreIndexFileDeactivatePropertyNotArray
        );
      }
    ...
    if (ArrayMethods.hasDuplicates(didUniqueSuffixes)) {
      throw new SidetreeError(
        ErrorCode.CoreIndexFileMultipleOperationsForTheSameDid
      );
    }
    ...
    if (!('provisionalIndexFileUri' in coreIndexFileModel)) {
      ...
      const createPlusRecoverOperationCount =
        createDidSuffixes.length + recoverDidSuffixes.length;
      ...
      InputValidator.validateCasFileUri(
        coreIndexFileModel.provisionalIndexFileUri,
