...
export default class ProvisionalIndexFile {
  ...
  public static async parse(
    provisionalIndexFileBuffer: Buffer
  ): Promise<ProvisionalIndexFile> {
    ...
    for (const property in provisionalIndexFileModel) {
      if (!allowedProperties.has(property)) {
        throw new SidetreeError(
          ErrorCode.ProvisionalIndexFileHasUnknownProperty
        );
      }
    }

    ProvisionalIndexFile.validateChunksProperty(
    ...
    if (didSuffixes.length > 0) {
      InputValidator.validateCasFileUri(
        provisionalIndexFileModel.provisionalProofFileUri,
        'provisional proof file URI'
      );
    } else {
    ...
    const provisionalIndexFile = new ProvisionalIndexFile(
      provisionalIndexFileModel,
      didSuffixes
    );
  ...
  private static validateOperationsProperty(operations: any): string[] {
    ...
    if (!Array.isArray(operations.update)) {
      throw new SidetreeError(
        ErrorCode.ProvisionalIndexFileUpdateOperationsNotArray
      );
    }

    // Validate all update operation references.
    InputValidator.validateOperationReferences(
      operations.update,
      'update reference'
    );

    // Make sure no operation with same DID.
    const didSuffixes = (operations.update as OperationReferenceModel[]).map(
      (operation) => operation.didSuffix
    );
    if (ArrayMethods.hasDuplicates(didSuffixes)) {
      throw new SidetreeError(
        ErrorCode.ProvisionalIndexFileMultipleOperationsForTheSameDid
      );
    }
  ...
  private static validateChunksProperty(chunks: any) {
    if (!Array.isArray(chunks)) {
      throw new SidetreeError(
        ErrorCode.ProvisionalIndexFileChunksPropertyMissingOrIncorrectType
      );
    }
    ...
    if (chunks.length !== 1) {
      throw new SidetreeError(
        ErrorCode.ProvisionalIndexFileChunksPropertyDoesNotHaveExactlyOneElement
      );
    }
    ...
    if (properties.length !== 1) {
      throw new SidetreeError(
        ErrorCode.ProvisionalIndexFileChunkHasMissingOrUnknownProperty
      );
    }
